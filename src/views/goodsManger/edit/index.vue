<template>
  <div id="create">
    <a-card>
      <a-form  :rules="rules" :model="goods">
        <a-form-item field="goodsSn" label="商品编号">
          <a-input :style="{width:'320px'}" v-model="goods.goodsSn" placeholder="请输入商品编号" />
        </a-form-item>
        <a-form-item field="name" label="商品名称">
          <a-input :style="{width:'320px'}" v-model="goods.name" placeholder="请输入商品名称" />
        </a-form-item>
        <a-form-item field="counterPrice" label="市场售价">
          <a-input :style="{width:'320px'}" v-model="goods.counterPrice" type="number" placeholder="0.00">
            <template #append>元</template>
          </a-input>
        </a-form-item>
        <a-form-item field="isNew" label="是否新品">
          <a-radio-group v-model="goods.isNew">
            <a-radio :value="true">新品</a-radio>
            <a-radio :value="false">非新品</a-radio>
          </a-radio-group>
        </a-form-item>
        <a-form-item field="isHot" label="是否热门">
          <a-radio-group v-model="goods.isHot">
            <a-radio :value="false">普通</a-radio>
            <a-radio :value="true">热门</a-radio>
          </a-radio-group>
        </a-form-item>
        <a-form-item field="isOnSale" label="是否在售">
          <a-radio-group v-model="goods.isOnSale">
          ...
          >
            <template #upload-button>
              <div
                ...
                v-if="goods && goods.picUrl"
              >
                <a-image :src="goods.picUrl" />
        ...
        <a-form-item label="商品单位">
          <a-input :style="{width:'320px'}" v-model="goods.unit" placeholder="件/个/盒" />
        </a-form-item>
        <a-form-item label="所属分类">
          <a-cascader v-model:model-value="goods.categoryId" :style="{width:'320px'}" :options="categoryList" expand-trigger="hover" clearable
                      @change="handleCategoryChange" />
        </a-form-item>
        <a-form-item label="商品介绍">
          <a-input :style="{width:'320px'}" v-model="goods.brief" />
        </a-form-item>
      </a-form>
    </a-card>
...
const goods = ref<GemallGoods>({ gallery: [] })
/**
 * 规格
 */
...
const submit = async () => {
  ...
  const res = await AdminGoodsControllerService.updateUsingPost(form.value)
  if (res.code === 0) {
...
const getGoodsDetail= async ()=>{
  const res = await AdminGoodsControllerService.detailUsingGet1(goodsId);
  if (res.code === 0) {
